...
Public Class DeviationRateForm
    ...
    Private Sub ChartForm_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Dim seriesRate5 As New Series()
        seriesRate5.Name = "seriesRate5"
        seriesRate5.ChartArea = "乖離率"
        seriesRate5.ChartType = SeriesChartType.Line        '' 折れ線のグラフ
        seriesRate5.BorderWidth = 1
        seriesRate5.XValueType = ChartValueType.Date        '' X軸は日付
        seriesRate5.YValueType = ChartValueType.Int64
        seriesRate5.Color = Color.Green
        seriesRate5.IsVisibleInLegend = False

        chartPrice.Series.Add(seriesRate5)

        Dim seriesRate25 As New Series()
        seriesRate25.Name = "seriesRate25"
        seriesRate25.ChartArea = "乖離率"
        seriesRate25.ChartType = SeriesChartType.Line        '' 折れ線のグラフ
        seriesRate25.BorderWidth = 1
        seriesRate25.XValueType = ChartValueType.Date        '' X軸は日付
        seriesRate25.YValueType = ChartValueType.Int64
        seriesRate25.Color = Color.Orange
        seriesRate25.IsVisibleInLegend = False

        chartPrice.Series.Add(seriesRate25)

        Dim seriesRate75 As New Series()
        seriesRate75.Name = "seriesRate75"
        seriesRate75.ChartArea = "乖離率"
        seriesRate75.ChartType = SeriesChartType.Line        '' 折れ線のグラフ
        seriesRate75.BorderWidth = 1
        seriesRate75.XValueType = ChartValueType.Date        '' X軸は日付
        seriesRate75.YValueType = ChartValueType.Int64
        seriesRate75.Color = Color.Red
        seriesRate75.IsVisibleInLegend = False

        chartPrice.Series.Add(seriesRate75)
        ...
        Dim areaRate = New ChartArea("乖離率")
        '' Y軸を合わせる処置
        areaRate.AlignWithChartArea = "株価"
        areaRate.AxisX.IsMarginVisible = False
        areaRate.AxisY.IsMarginVisible = False
        areaRate.AxisY.MajorGrid.Enabled = True
        areaRate.Axes(0).IsLabelAutoFit = False
        'areaRate.Axes(0).Enabled = AxisEnabled.False
        areaRate.IsSameFontSizeForAllAxes = False

        chartPrice.ChartAreas.Add(areaRate)
    ...
    Private Sub DisplayChart(証券コード As Decimal)
        ...
        Dim dpRate5 As DataPoint
        Dim dpRate25 As DataPoint
        Dim dpRate75 As DataPoint
        ...
        For Each r In prices
            ...
            If r.移動平均5.HasValue Then
                ...
                dpRate5 = New DataPoint
                dpRate5.SetValueXY(r.日付, r.移動平均5乖離率 * 100)
                Me.chartPrice.Series.Item("seriesRate5").Points.Add(dpRate5)
            End If
            ...
            If r.移動平均25.HasValue Then
                ...
                dpRate25 = New DataPoint
                dpRate25.SetValueXY(r.日付, r.移動平均25乖離率 * 100)
                Me.chartPrice.Series.Item("seriesRate25").Points.Add(dpRate25)
            End If
            ...
            If r.移動平均75.HasValue Then
                ...
                dpRate75 = New DataPoint
                dpRate75.SetValueXY(r.日付, r.移動平均75乖離率 * 100)
                Me.chartPrice.Series.Item("seriesRate75").Points.Add(dpRate75)
            End If
