...
Public Class StockPriceDA
    ...
    Public Function SelectFilter() As (DBResult, List(Of StockPriceBI))
        Dim stbSql As New StringBuilder
        stbSql.Append("  SELECT  ")
        stbSql.Append("         証券コード ")
        stbSql.Append("       , 企業名 ")
        ...
        stbSql.Append("           FROM StockPrice ")
        stbSql.Append("        ) AS TBL  ")
        stbSql.Append("  WHERE CASE WHEN 移動平均5件数   = 5   THEN CAST( (調整後終値 - 移動平均5)   AS REAL ) / 移動平均5   ELSE NULL END  <= -0.1 ")
        stbSql.Append("    AND CASE WHEN 移動平均75件数  = 75  THEN CAST( (調整後終値 - 移動平均75)  AS REAL ) / 移動平均75  ELSE NULL END  <= -0.3 ")
        stbSql.Append("    AND CASE WHEN 移動平均200件数 = 200 THEN CAST( (調整後終値 - 移動平均200) AS REAL ) / 移動平均200 ELSE NULL END  <= -0.3 ")
        stbSql.Append("   ORDER BY 証券コード ")
        stbSql.Append("          , 日付 ")
        Return Me.DBSelect(Of StockPriceBI)(stbSql.ToString())
    End Function
End Class
